...
class etc-updates {
    ...
    file{'/etc/cron.weekly/etc-updates.cron':
        source => "puppet://$server/modules/etc-updates/etc-updates.cron",
    ...
    file{'/etc/sysconfig/etc-update.conf':
        source => "puppet://$server/modules/etc-updates/etc-update.conf",
        mode => 0644, owner => root, group => 0;
    }

    # etc-update
    file{'/usr/local/sbin/etc-update':
        source => "puppet://$server/modules/etc-updates/etc-update",
        mode => 0700, owner => root, group => 0;
    }

    # etc-downgrade
    file{'/usr/local/sbin/etc-downgrade':
        source => "puppet://$server/modules/etc-updates/etc-downgrade",
        mode => 0700, owner => root, group => 0;
    }
}
